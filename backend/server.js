...
passport.deserializeUser(async (id, done) => {
    try {
        const result = await pool.query('SELECT user_id, user_name, user_role, avatar_url FROM PUBLIC.SUMA_users WHERE user_id = $1', [id]);
...
app.get('/auth/microsoft/callback', 
  ...
  failureRedirect: process.env.FRONTEND_URL || 'https://suma-pyrl.vercel.app'
  })
);
